...
Manifold::GeodesicPtr SurfaceOfRevolution<SurfaceOfRevolution2d>::Portal::getGeodesic(IntersectionPtr intersection) {
	...
	orientation[0,3] = 1;
	orientation.block<3,3>(1,0) = intersection->getOrientation();
	Vector4d p;
	p << -1, intersection->getPosition();
...
IntersectionPtr SurfaceOfRevolution<SurfaceOfRevolution2d>::Portal::getIntersection(Manifold::GeodesicPtr geodesic) {
	...
	Vector4d p;
	p << -1,((SurfaceOfRevolution<SurfaceOfRevolution2d>::Point*)end->getPosition().get())->getSpherical();
	Matrix4d orientation = castedEnd->getOrientation();
	std::cout << "orientation before:\n" << orientation << std::endl;
	Matrix4d reflection = Matrix4d::Identity() - p*p.transpose();
	orientation = reflection*orientation;
	//A second reflection on the 3 axis would get the inverse of the rotation in the last method, but since we're ignoring that part of the matrix, there's no point in doing it. I just changed the tests to look for positive 1 instead of negative.
	std::cout << "orientation after:\n" << orientation << std::endl;
	assert((orientation.block<1,4>(0,0).transpose()-Vector4d(0,0,0,1)).squaredNorm() < EPSILON);
	assert((orientation.block<4,1>(0,3)-Vector4d(1,0,0,0)).squaredNorm() < EPSILON);
	assert(fabs(orientation.block<3,3>(1,0).determinant()-1) < EPSILON);
	return IntersectionPtr(new Intersection(p.end<3>(), orientation.block<3,3>(1,0), geodesic->getVector()-vector));
}
